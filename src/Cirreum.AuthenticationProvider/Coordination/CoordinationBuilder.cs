...
/// <summary>
/// Configures the atomic-coordination backend shared by the authentication schemes that need it —
/// <see cref="IReplayGuard"/> for the SignedRequest strict-nonce posture and <see cref="IRequestThrottle"/>
/// for the ApiKey <c>SelfContained</c> conformance profile. The backend is registered once (via
/// <c>AddCoordination</c>) and shared; schemes consume whichever interface they need. Distributed adapters
/// (e.g. <c>Cirreum.Authentication.Coordination.Redis</c>) contribute their own <c>UseXxx()</c> extension
/// methods on this type, keeping the scheme packages blind to the backing store.
/// </summary>
